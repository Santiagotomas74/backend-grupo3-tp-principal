...
// NOTA HACER UN METODO PRIVADO PARA VALIDAR LAS REGLAS DE NEGOCIO CRUZADAS Y LLAMARLO DESDE LOS DOS LUGARES (CREACION Y EDICION) PARA NO REPETIR CODIGO
...
public OrdenCargaResponseDTO editarOrdenCarga(Integer id, OrdenCargaRequestDTO dto) {
    ...
    if (estadoActual.equalsIgnoreCase("Entregada") || estadoActual.equalsIgnoreCase("Cancelada")) {
        throw new IllegalArgumentException("No se puede editar una orden que ya se encuentra en estado '" + estadoActual + "'.");
    }
    ...
    if (!camion.getId().equals(orden.getCamion().getId()) && !camion.getEstado().getNombre().equalsIgnoreCase("Disponible")) {
        throw new IllegalArgumentException("El nuevo camión seleccionado no está disponible.");
    }

    if (!acoplado.getId().equals(orden.getAcoplado().getId()) && !acoplado.getEstado().getNombre().equalsIgnoreCase("Disponible")) {
