        ...
        public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
                ...
                var authorities = usuario.getRoles().stream()
                                .flatMap(rol -> Stream.concat(
                                                Stream.of(new SimpleGrantedAuthority("ROLE_" + rol.getNombre())), // Adds
                                                                                                                  // ROLE_ADMIN
                                                rol.getPermisos().stream()
                                                                .map(p -> new SimpleGrantedAuthority(p.getCodigo())) // Adds
                                                                                                                     // CAN_READ
                                ))
                                .collect(Collectors.toSet());

                return User.builder()
                                .username(usuario.getEmail()) // por ahora se esta usando el mail como username. sujeto
                                                              // a cambios
