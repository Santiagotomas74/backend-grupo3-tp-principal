...
public class TransportistaService {
  ...
  public void registrarNuevoTransportista(AltaTransportistaRequestDTO dto) {
    ...
    var tipoVinculo = tipoVinculoRepository.findById(dto.tipoVinculoId())
        .orElseThrow(
            () -> new IllegalArgumentException("El Tipo de Vinculo especificado no existe: " + dto.tipoVinculoId()));

    String nombreVinculo = tipoVinculo.getNombre();

    // Si es tercerizado la empresa es obligatoria. Sino no puede estar vinculado a
    // una empresa.
    if (nombreVinculo.equalsIgnoreCase("Tercerizado")) {
      if (dto.empresaId() == null) {
        throw new IllegalArgumentException(
            "Debe especificar una empresa obligatoriamente para el tipo de Vinculo Tercerizado");
      }
    } else {
      if (dto.empresaId() != null)
        throw new IllegalArgumentException(
            "Un Transportista con vinculo '" + nombreVinculo + "' no puede estar asociado a una empresa tercerizada");
    }
    ...
    var empresa = dto.empresaId() != null
        ? empresaTercerizadaRepository.findById(dto.empresaId())
            .orElseThrow(
                () -> new IllegalArgumentException("La Empresa Tercerizada especificada no existe: " + dto.empresaId()))
        : null;
    ...
    if (dto.documentos() != null && !dto.documentos().isEmpty()) {
      for (DocumentoRequestDTO docDto : dto.documentos()) {
        var tipoDocumento = tipoDocumentoRepository.findById(docDto.tipoDocumentoId())
            .orElseThrow(
                () -> new IllegalArgumentException("El Tipo de Documento no existe: " + docDto.tipoDocumentoId()));
