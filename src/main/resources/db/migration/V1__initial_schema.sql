...
CREATE TABLE Documentacion (
    ...
    FOREIGN KEY (acoplado_id) REFERENCES Acoplado(id),
    CHECK (
        transportista_id IS NOT NULL
        OR camion_id IS NOT NULL
        OR acoplado_id IS NOT NULL
    )
);
...
-- INDICES EXTRA
-- =========================
CREATE INDEX idx_usuario_lugar_operativo ON Usuario(lugar_operativo_id);

CREATE INDEX idx_transportista_empresa ON Transportista(empresa_id);

CREATE INDEX idx_vehiculo_empresa ON Vehiculo(empresa_id);

CREATE INDEX idx_acoplado_empresa ON Acoplado(empresa_id);

CREATE INDEX idx_orden_estado ON Orden_Carga(estado_id);

CREATE INDEX idx_orden_transportista ON Orden_Carga(transportista_id);

CREATE INDEX idx_documentacion_vencimiento ON Documentacion(fecha_vencimiento);

CREATE INDEX idx_incidencia_orden ON Incidencia(orden_id);

CREATE INDEX idx_auditoria_orden ON Auditoria_Estado(orden_id);